...
- **SSH-based** (clones and normalizes remotes to `git@<host>:<org>/<repo>.git`)
...
2. Clone if missing using the canonical SSH URL (`git@<host>:<org>/<repo>.git`).
3. Reuse existing checkout if present.
4. Normalize `origin` to the canonical SSH URL.
