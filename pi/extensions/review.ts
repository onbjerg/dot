 ...
 * Supports multiple review modes using jj/Jujutsu:
 * - Review a GitHub pull request (fetches the PR head into jj and creates a working-copy child)
 * - Review against a base bookmark (PR style)
 * - Review the current jj working-copy commit
 * - Review a specific jj revision
 * - Shared custom review instructions (applied to all review modes when configured)
 *
 * Usage:
 * - `/review` - show interactive selector
 * - `/review pr 123` - review PR #123 (fetches locally via jj)
 * - `/review pr https://github.com/owner/repo/pull/123` - review PR from URL
 * - `/review uncommitted` - review the current jj working-copy commit directly
 * - `/review bookmark main` (or `/review branch main`) - review against the main bookmark
 * - `/review commit abc123` - review specific jj revision
 ...
 *
 * Note: PR review requires an empty jj working-copy commit (`jj diff -r @` has no output).
 */
...
// Prompts (adapted from Codex for jj/Jujutsu)
const UNCOMMITTED_PROMPT =
	"Review the current jj working-copy commit changes. Run `jj status` and `jj diff -r @ --git` to inspect the changes represented by `@`. Provide prioritized findings.";

const LOCAL_CHANGES_REVIEW_INSTRUCTIONS =
	"Also include local working-copy changes from the current jj change. Use `jj status` and `jj diff -r @ --git` so local fixes are part of this review cycle.";

const BASE_BRANCH_PROMPT_WITH_MERGE_BASE =
	"Review the code changes against the base bookmark '{baseBranch}'. The fork-point/merge-base commit for this comparison is {mergeBaseSha}. Run `jj diff --from {mergeBaseSha} --to @ --git` to inspect the changes relative to {baseBranch}. Provide prioritized, actionable findings.";

const BASE_BRANCH_PROMPT_FALLBACK =
	"Review the code changes against the base bookmark '{branch}'. Start by finding the fork point between the current jj working-copy commit and {branch} (for example, `jj log -r 'fork_point(@ | {branch})' --no-graph -T 'commit_id.short() ++ \"\\n\"'`), then run `jj diff --from <fork-point> --to @ --git` to see the changes that would be merged. Provide prioritized, actionable findings.";

const COMMIT_PROMPT_WITH_TITLE =
	'Review the code changes introduced by jj revision {sha} ("{title}"). Run `jj show -r {sha} --git` to inspect it. Provide prioritized, actionable findings.';

const COMMIT_PROMPT = "Review the code changes introduced by jj revision {sha}. Run `jj show -r {sha} --git` to inspect it. Provide prioritized, actionable findings.";

const PULL_REQUEST_PROMPT =
	'Review pull request #{prNumber} ("{title}") against the base bookmark \'{baseBranch}\'. The fork-point/merge-base commit for this comparison is {mergeBaseSha}. Run `jj diff --from {mergeBaseSha} --to @ --git` to inspect the changes that would be merged. Provide prioritized, actionable findings.';

const PULL_REQUEST_PROMPT_FALLBACK =
	'Review pull request #{prNumber} ("{title}") against the base bookmark \'{baseBranch}\'. Start by finding the fork point between the current jj working-copy commit and {baseBranch} (for example, `jj log -r \'fork_point(@ | {baseBranch})\' --no-graph -T \'commit_id.short() ++ "\\n"\'`), then run `jj diff --from <fork-point> --to @ --git` to see the changes that would be merged. Provide prioritized, actionable findings.';
...
/**
 * Resolve the first jj revset/revision expression that names exactly one visible commit.
 */
async function resolveRevision(pi: ExtensionAPI, candidates: Array<string | undefined>): Promise<string | null> {
	for (const candidate of candidates) {
		const revset = candidate?.trim();
		if (!revset) continue;
...
/**
 * Get the jj fork point (merge base) between @ and a base bookmark/revision.
 */
async function getMergeBase(
	...
	try {
		const baseRev = await resolveRevision(pi, [branch, `${branch}@origin`]);
		...
		const mergeBase = stdout.trim().split("\n").filter(Boolean)[0];
		if (code === 0 && mergeBase) {
			return mergeBase;
		}
...
/**
 * Get list of local jj bookmarks. Kept under the old function name to minimize UI churn.
 */
...
/**
 * Get list of recent jj revisions on the current ancestry.
 */
async function getRecentCommits(pi: ExtensionAPI, limit: number = 10): Promise<Array<{ sha: string; title: string }>> {
	...
	return stdout
		...
		.map((line) => {
			...
			return { sha, title: rest.join("\t") || "(no description)" };
		});
}

/**
 * Check if the jj working-copy commit (@) has changes against its parent.
 */
async function hasUncommittedChanges(pi: ExtensionAPI): Promise<boolean> {
	const { stdout, code } = await pi.exec("jj", ["diff", "--summary", "-r", "@"]);
	return code === 0 && stdout.trim().length > 0;
}

/**
 * Check if there are working-copy changes that would be inconvenient to move away from.
 */
...
/**
 * Fetch and enter a PR using jj. This works for same-repository PR branches fetchable from origin.
 */
async function checkoutPr(pi: ExtensionAPI, prNumber: number, prInfo: PullRequestInfo): Promise<{ success: boolean; error?: string }> {
	await pi.exec("jj", ["git", "fetch", "--remote", "origin", "--branch", prInfo.baseBranch]);
	const { stdout, stderr, code } = await pi.exec("jj", [
		"git",
		"fetch",
		"--remote",
		"origin",
	...
	if (code !== 0) {
		return {
			success: false,
			error: stderr || stdout || `Failed to fetch PR #${prNumber} head bookmark '${prInfo.headBranch}' from origin`,
		};
	}

	const headRev = await resolveRevision(pi, [`${prInfo.headBranch}@origin`, prInfo.headBranch, prInfo.headOid]);
	if (!headRev) {
		return { success: false, error: `Fetched PR #${prNumber}, but could not resolve its jj revision` };
	}

	const newResult = await pi.exec("jj", ["new", headRev]);
	if (newResult.code !== 0) {
		return { success: false, error: newResult.stderr || newResult.stdout || "Failed to create jj working-copy child for PR" };
	}
...
/**
 * Get the closest local jj bookmark in @'s ancestry.
 */
...
/**
 * Get the default bookmark (main, master, or trunk)
 */
async function getDefaultBranch(pi: ExtensionAPI): Promise<string> {
	...
	const trunk = await resolveRevision(pi, ["trunk()"]);
...
async function buildReviewPrompt(
	...
	switch (target.type) {
		...
		case "pullRequest": {
			const mergeBase = await getMergeBase(pi, `${target.baseBranch}@origin`);
			const basePrompt = mergeBase
				? PULL_REQUEST_PROMPT
...
const REVIEW_PRESETS = [
	{ value: "uncommitted", label: "Review current jj working-copy commit", description: "" },
	{ value: "baseBranch", label: "Review against a base bookmark", description: "(local)" },
	{ value: "commit", label: "Review a jj revision", description: "" },
	{ value: "pullRequest", label: "Review a pull request", description: "(GitHub PR via jj)" },
...
export default function reviewExtension(pi: ExtensionAPI) {
	...
	async function getSmartDefault(): Promise<"uncommitted" | "baseBranch" | "commit"> {
		// Priority 1: If the working-copy commit has changes, default to reviewing them
		if (await hasUncommittedChanges(pi)) {
			return "uncommitted";
		}

		// Priority 2: If the closest bookmark is not the default bookmark, default to PR-style review
	...
	async function showBranchSelector(ctx: ExtensionContext): Promise<ReviewTarget | null> {
		...
		// Never offer the current closest bookmark as a base bookmark (reviewing against itself is meaningless).
		const candidateBranches = currentBranch ? branches.filter((b) => b !== currentBranch) : branches;

		if (candidateBranches.length === 0) {
			ctx.ui.notify(
				currentBranch ? `No other bookmarks found (current bookmark: ${currentBranch})` : "No bookmarks found",
				"error",
			);
			return null;
		}

		// Sort bookmarks with default bookmark first
	...
	/**
	 * Show PR input and fetch/enter it with jj
	 */
	async function showPrInput(ctx: ExtensionContext): Promise<ReviewTarget | null> {
		// First check for pending changes that would be inconvenient to move away from.
		if (await hasPendingChanges(pi)) {
			ctx.ui.notify("Cannot enter PR review: the jj working-copy commit has changes. Commit, split, or abandon them first.", "error");
			return null;
		}
		...
		// Fetch the PR and move the jj working copy to a child of the PR head
		ctx.ui.notify(`Fetching PR #${prNumber} into jj...`, "info");
		const checkoutResult = await checkoutPr(pi, prNumber, prInfo);

		if (!checkoutResult.success) {
			ctx.ui.notify(`Failed to enter PR review: ${checkoutResult.error}`, "error");
			return null;
		}

		ctx.ui.notify(`Entered PR #${prNumber} (${prInfo.headBranch}) in jj`, "info");
	...
	async function executeReview(
		...
		if (useFreshSession) {
			...
			if (firstUserMessage) {
				// Navigate to first user message to create a new conversation branch from that point
	...
	/**
	 * Fetch/enter a PR and return a ReviewTarget (or null on failure)
	 */
	...
	async function runLoopFixingReview(
		...
		try {
			ctx.ui.notify(
				"Loop fixing enabled: using Empty review branch mode and cycling until no blocking findings remain.",
				"info",
			);
	...
	pi.registerCommand("review", {
		description: "Review jj code changes (PR, working-copy commit, bookmark, revision, or folder)",
		handler: async (args, ctx) => {
			...
			if (parsed.target) {
				if (parsed.target.type === "pr") {
					// Handle PR fetch/checkout (async operation)
			...
			while (true) {
				...
				if (messageCount > 0) {
					...
					const choice = await ctx.ui.select("Start review in:", ["Empty review branch", "Current session"]);
					...
					useFreshSession = choice === "Empty review branch";
				}
	...
	const REVIEW_SUMMARY_PROMPT = `We are leaving a code-review conversation branch and returning to the main coding conversation branch.
Create a structured handoff that can be used immediately to implement fixes.

You MUST summarize the review that happened in this conversation branch so findings can be acted on.
	...
	async function navigateWithSummary(
		...
		if (showLoader && ctx.hasUI) {
			return ctx.ui.custom<{ cancelled: boolean; error?: string } | null>((tui, theme, _kb, done) => {
				const loader = new BorderedLoader(tui, theme, "Returning and summarizing review conversation branch...");
	...
	async function executeEndReviewAction(
		...
		if (!originId) {
			if (!getReviewState(ctx)?.active) {
				ctx.ui.notify("Not in a review conversation branch (use /review first, or review was started in current session mode)", "info");
			}
			return "error";
		}
